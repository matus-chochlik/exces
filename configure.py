...
def get_argument_parser():
	...
	argparser = argparse.ArgumentParser(
		prog="configure",
		description="""
			Configuration script for the exces library (version %(version)s)
		""" % { "version" : version },
		epilog="""
...
def main(argv):
	...
	if(not options.build_examples):
		cmake_options.append("-DEXCES_NO_EXAMPLES=On")

	# disable building the docs
	if(not options.build_docs):
		cmake_options.append("-DEXCES_NO_DOCS=On")
	...
	if(options.with_tests):
		cmake_options.append("-DEXCES_WITH_TESTS=On")
		...
		if(cmake_build_tool):
			...
			if(not options.build):
				if(cmake_build_tool == "make"):
					print("# To build exces do the following:")
					print(str())
	...
	if options.build:
		...
		if build_tool_name in ("make",):
			...
			build_cmd_line = [
				cmake_build_tool,
				os.path.join(options.build_dir, "exces.sln"),
				"/Build",
				"Release"
			]
